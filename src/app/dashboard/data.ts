...
export async function fetchDashboardData(licenseKey: string): Promise<DashboardData> {
  const days = getLast30Days()
  const r = getRedis()

  // --- Chart 1: Score Trend (per-license) ---
  const scoreTrend: ScoreTrendPoint[] = await Promise.all(
    days.map(async (day) => {
      ...
      const events = await Promise.all(
        requestIds.map(async (id) => {
          try {
            ...
            if (!raw) return null
            const parsed: UsageEvent = typeof raw === 'string' ? JSON.parse(raw) : (raw as unknown as UsageEvent)
            return parsed
          } catch {
            return null
          }
  ...
  // --- Chart 2: Agent-Ready Funnel (global, not per-license) ---
